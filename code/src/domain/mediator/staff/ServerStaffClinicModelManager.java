...
public class ServerStaffClinicModelManager extends UnicastRemoteObject
      implements RemoteStaffClinicModel
{
   ...
   public ServerStaffClinicModelManager()
   ...
   public void addOneEmployee(String[] employeeData)
   {
      ...
      {
         username = username.substring(0, 1).toLowerCase()
               + employeeData[1].toString().toLowerCase();
      }
      String password = "123456";

      employee = EmployeeFactory.create(id, firstName, lastName, dob, startDate,
            telNumber, email, gender, employeeType, username, password);
